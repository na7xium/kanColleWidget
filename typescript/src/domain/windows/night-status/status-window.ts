...
module KCW {
    /*
    export class NightShipsStatusWindow extends Infra.WindowFrame {
        private static created: NightShipsStatusWindow[] = [];
        private instance: Window;
        constructor(params: WinMakeParams, imgURI: string) {
            super(
                NightShipsStatusWindow.url() + "?imgURI=" + imgURI,
                "_blank",
        ...
        public static show(opt: any = {}) {
            ...
            WindowFinder.findKCWidget().done((win: chrome.windows.Window) => {
                Infra.Capture.whole(win.id).done((imgURI: string) => {
                    imgURI = NightShipsStatusWindow.trim(imgURI, opt);
                    var params: WinMakeParams = NightShipsStatusWindowRepository.local().restore();
        ...
        private register() {
            NightShipsStatusWindow.created.push(this);
        }
        private close() {
            if (this.instance && this.instance.close) this.instance.close();
        }
        public static sweep() {
            for (var i = 0; i < NightShipsStatusWindow.created.length; i++) {
                NightShipsStatusWindow.created[i].close();
            }
            NightShipsStatusWindow.created = [];
        }
        private static trim(imgURI: string, opt: any = {}): string {
            var params = NightShipsStatusWindow.calcOpenParams(imgURI, opt);
